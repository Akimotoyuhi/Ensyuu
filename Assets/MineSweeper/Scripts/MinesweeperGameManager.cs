...
{/// <summary>行（縦）</summary>
    ...
    {
        _vec = new Vector3(_enemy.transform.position.x + Random.Range(-50f, 50f), _enemy.transform.position.y + Random.Range(-50f, 50f));
        _quat = new Quaternion(_damagePrefab.transform.rotation.x, _damagePrefab.transform.rotation.y, _damagePrefab.transform.rotation.z + Random.Range(0f, 360f), _damagePrefab.transform.rotation.w);
        GameObject t =  Instantiate(_damagePrefab, _vec, _quat);
